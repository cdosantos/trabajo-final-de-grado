...
# ---------- args ------------
# ./scripts/run-evosuite-test.sh relative_path class package
# example: ./scripts/run-evosuite-test.sh SimpleMethods_getMin/ SimpleMethods "getMin\(int,int\)" examples
...
mutants=0
detected_mutants=0
echo "mutant_id,mutation,failing_test,assertions_failures,non_assertions_failures" > ${output_dir}evosuite-result.csv
...
do
  ...
  out=$(timeout --foreground 1m java -cp $mutants_dir:${subject_jar}:${output_dir}evosuite/bin:$junit:$hamcrest:$EVOSUITE_STANDALONE org.junit.runner.JUnitCore $tests_files)
  ...
  echo $mutants,$mutation,$failing_test,$assertions_failures,$non_assertions_failures >> ${output_dir}evosuite-result.csv
done
echo $2.$method_without_args,$tests,$mutants,$detected_mutants >> ${output_dir}../../../evosuite-all-results.csv
